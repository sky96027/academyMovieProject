...
<style type="text/css">
.carousel-inner { display: table; margin-left: auto; margin-right: auto; }
.carousel { height : 350px; width: 500px; margin: 0 auto; overflow: hidden; background-color: gray;}
.carousel-item{ line-height: 350px;}
.carousel-item img{vertical-align: middle; }
.modify-delete {text-align:right;}
</style>
<script src="<%=request.getContextPath()%>/resources/js/comment.js"></script>
</head>
<body>
<div class="body container">
		<c:if test="${movieboard != null}">
			<h1>리뷰 상세</h1>
			<br>
			<div class="form-group">
			  <h2>${movieboard.mv_name}</h2>
			  <h6>개봉일 : ${movieboard.mv_reg_date_str}</h6> 
			  <h6>장르 : ${movieboard.mv_genre}</h6>
			  <h6>러닝 타임 : ${movieboard.mv_time}분</h6>
			  <h6>관람 등급 : ${movieboard.mv_age}세 이용가</h6>
			  <h6>감독 : ${movieboard.mv_director}</h6>
			</div>
			<div class="form-group">
					  ...
					  <div class="carousel-inner">
					  	<c:forEach items="${files}" var="file" varStatus="vs">
						    <div class="carousel-item <c:if test="${vs.index == 0}">active</c:if>">
						      <img src="<c:url value="/img${file.fi_name}"></c:url>" >
						    </div>
					    </c:forEach>
					  </div>
			...
			<div class="form-group">
			  <div class="form-control" style="min-height:500px; height:auto;">${movieboard.mv_outline}</div>
			</div>
			...
			<c:if test="${(user != null && user.me_id == movieboard.mv_me_id) 
				|| user.me_authority == '최고 관리자' || user.me_authority == '관리자'}">
				<div class="modify-delete">
					<a href="<%=request.getContextPath()%>/movieboard/modify?mv_num=${movieboard.mv_num}">
						<button class="btn btn-outline-success">수정</button>
					</a>
					<a href="<%=request.getContextPath()%>/movieboard/delete?mv_num=${movieboard.mv_num}">
	...
	<script>
		...
		$(function(){
			...
			$('.btn-comment').click(function(){
				...
				//게시글 번호를 가져오기
				var co_mv_num = '${movieboard.mv_num}';
				//댓글 원본 번호(나중에)
		...
		$('.btn-up, .btn-down').click(function(){
			...
			var li_mv_num = '${movieboard.mv_num}';
			...
			$.ajax({
				...
				url: '<%=request.getContextPath()%>/movieboard/likes',
				dataType:"json",
		...
		function viewLikes(likes){
			$.ajax({
				...
				type:'POST',
				data: JSON.stringify(likes),
				url: '<%=request.getContextPath()%>/movieboard/view/likes',
				dataType:"json",
				contentType:"application/json; charset=UTF-8",
				success : function(res){
					$('.likes-btn-box .btn')
		...
		function readComment(co_mv_num, page){
			//게시글 번호가 없으면 가져올 게시글이 없어서 작업하지 않음
			if(co_mv_num != ''){
				var url = '/comment/list?co_mv_num='+co_mv_num + '&page='+page;
